    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          // 음식점 데이터를 가까운 순으로 정렬하고 각 장소에 거리 값을 추가
          const sortedList = places.map((place) => {
            if (place.rstrLatPos && place.rstrLotPos) { // 좌표 값이 있는지 확인
              const distance = calculateDistance(
                userLat,
                userLon,
                place.rstrLatPos, // double 값을 그대로 사용
                place.rstrLotPos  // double 값을 그대로 사용
              );
              return {
                ...place,
        ...
        (error) => {
          ...
          setSortedPlaces(places); // 기본 음식점 리스트로 설정
        }
      );
    } else {
...
const toggleSortOrder = () => {
  ...
  const handleAlphabeticalSort = () => {
    if (isLocationAllowed) {
      ...
      const sortedByAlphabetWithDistance = [...sortedPlaces].sort((a, b) => a.rstrNm.localeCompare(b.rstrNm));
      ...
      const sortedByAlphabet = [...places].sort((a, b) => a.rstrNm.localeCompare(b.rstrNm));
...
export default NearbyFoodPlace;